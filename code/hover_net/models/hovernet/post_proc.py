...
####
def get_per_class_prob(type_list, type_pixels, nr_types):
    per_class_prob = np.zeros(nr_types)
    for i, cell_type in enumerate(type_list):
        per_class_prob[cell_type] = type_pixels[i] / (np.sum(type_pixels) + 1.0e-6)
...
def process(pred_map, nr_types=None, return_centroids=False):
    ...
    if nr_types is not None:
        ...
        for inst_id in list(inst_info_dict.keys()):
            ...
            #inst_prob_crop = pred_prob[rmin:rmax, cmin:cmax]
            inst_map_crop = (
                inst_map_crop == inst_id
            )  # TODO: duplicated operation, may be expensive
            inst_type = inst_type_crop[inst_map_crop]
            #per_class_prob = inst_prob_crop[inst_map_crop]
            ...
            #inst_info_dict[inst_id]["prob_per_class"] = list(get_per_class_prob(type_list, type_pixels, nr_types))
