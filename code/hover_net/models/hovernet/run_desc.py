...
def train_step(batch_data, run_info):
    ...
    if loss_opts["class_weights"] is not None:
        loss_opts["class_weights"] = torch.tensor(loss_opts["class_weights"]).to("cuda")
    ...
    for branch_name in pred_dict.keys():
        for loss_name, loss_weight in loss_opts[branch_name].items():
            ...
            if (branch_name == "tp" and loss_name == 'bce'):
                loss_args.append(loss_opts["class_weights"])
...
def valid_step(batch_data, run_info):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        # Change in the original repo to output all class propability   
...
def infer_step(batch_data, model):
    ...
    with torch.no_grad():  # dont compute gradient
        ...
        pred_dict["np"] = F.softmax(pred_dict["np"], dim=-1)[..., 1:]
        if "tp" in pred_dict:
            type_prob = F.softmax(pred_dict["tp"], dim=-1)
            type_map = torch.argmax(type_prob, dim=-1, keepdim=True)
            type_map = type_map.type(torch.float32)
            pred_dict["tp"] = type_map
            #pred_dict['prob'] = type_prob
        pred_output = torch.cat(list(pred_dict.values()), -1)
