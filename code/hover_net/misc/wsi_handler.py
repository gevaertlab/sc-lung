...
class OpenSlideHandler(FileHandler):
    ...
    def __load_metadata(self):
        ...
        if "aperio.AppMag" in wsi_properties:
            level_0_magnification = wsi_properties.get('aperio.AppMag')
        elif "openslide.mpp-x" in wsi_properties:
            level_0_magnification = round(0.5 / float(wsi_properties.get('openslide.mpp-x')) * 20)
        else:
            level_0_magnification = 40
            print(f"can not found magnification, use 40X for default !!!!")
