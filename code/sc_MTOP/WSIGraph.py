# coding=utf-8
"""
Codes adapted from Zhao et al. Nat Comm, 2023, with my own customizations for incorporating new features and additional cell types
...
def getMorphFeatures(name, types, contours, bboxes, desc, process_n=1):
...
def save_PIL_image(img, path, image_type = "tiff", downsample = 1):
    if isinstance(img, np.ndarray):
        img = Image.fromarray(img)
    if downsample != 1:
        new_width = img.width // downsample
        new_height = img.height // downsample
...
def getCellImg(slidePtr, bbox, pad=2, level=0):
    ...
    bbox[0] = bbox[0] - pad
    bbox[1] = bbox[1] + pad
    if isinstance(slidePtr, Image.Image):
        cellImg = slidePtr.crop((bbox[0][0],bbox[0][1],bbox[1][0],bbox[1][1]))
    else:
...
def getGraphCenterFeatures(graph: ig.Graph):
    ...
    norm_cmds = ['name', 'Nsubgraph', 'Degrees','Coreness', 'ClusteringCoefficient']
...
def constructGraphFromDict(
        ...
        knn_n: int = 5, level: int = 0, offset=np.array([0, 0]), n_process = 1,
        dataset= "pannuke", cal_morph = True, cal_texture = True, cal_graph = True, 
    ...
    graph_dataset = get_dataset(dataset)
    offset = np.array([0, 0])
    print(f"{'Total 9 steps: 0 ~ 8':*^30s}")
    mag = nucleusInfo['mag']

    if mag is None:
        print("Can not find WSI magnification, using default 40X")
        mag = 40

    distanceThreshold = distanceThreshold / (40.0 / mag)

    bboxes, centroids, contours, types = [], [], [], []
    #nucleusInfo = replace_type_label(nucleusInfo)
    ...
    if cal_morph:
        ...
        t1 = time.time()
        if len(restrict_type) > 0:
            indices = [index for index, value in enumerate(types) if value in restrict_type]
            target_names = [names[index] for index in indices]
            target_contours = [contours[index] for index in indices]
            target_bboxes = [bboxes[index] for index in indices]
            target_centroids = [centroids[index] for index in indices]
            target_types = [types[index] for index in indices]
            morphFeats = getMorphFeatures(target_names, target_types, target_contours, target_bboxes, \
                                        'MorphFeatures', process_n=n_process)
        else:
            morphFeats = getMorphFeatures(names, types, contours, bboxes, 'MorphFeatures', process_n=n_process)
    ...
    if cal_texture:
        ...
        t2 = time.time()
        if len(restrict_type) > 0:
            GLCMFeats = getGLCMFeatures(wsiPath, target_names, target_contours, \
                                        target_bboxes, pad=2, level=level, process_n=n_process)
        else:
            GLCMFeats = getGLCMFeatures(wsiPath, names, contours, bboxes, pad=2, level=level, process_n=n_process)
    ...
    # Need to be changed along with HoverNet_type
    featype_dict, epithIDs, lymphIDs, fibroIDs, macroIDs, vascuIDs = graph_dataset.get_featype_dict(types)
    edge_info = pd.DataFrame({'source': [], 'target': [], 'featype': []})

    if cal_graph:
        t3 = time.time()
        if len(restrict_edge) > 0:
            featype_dict = {k:v for k, v in featype_dict.items() if k in restrict_edge}

        for featype, featype_index_list in zip(featype_dict.keys(),featype_dict.values()): 
            ...
            for src, i_src in enumerate(featype_index_list):
                for tar, i_tar in enumerate(featype_index_list):
                    if src != tar:
                        ...
                        if i_src == i_tar:
                            disknn, vertex_index = tree.query(centroid_src, k=knn_n + 1, distance_upper_bound=distanceThreshold, p=2)                      
                            disknn = disknn[..., -knn_n:]
                            vertex_index = vertex_index[..., -knn_n:]
                        else:
                            disknn, vertex_index = tree.query(centroid_src, k=knn_n, distance_upper_bound=distanceThreshold, p=2)
    ...
    t4 = time.time()
    centroid_T = globalGraph.induced_subgraph(epithIDs).vs['Centroid']
    centroid_L = globalGraph.induced_subgraph(lymphIDs).vs['Centroid']
    centroid_F = globalGraph.induced_subgraph(fibroIDs).vs['Centroid']
    Ttree = cKDTree(centroid_T)
    STree = cKDTree(centroid_F)

    # blocker for lymphocyte
    dis, pairindex_T = Ttree.query(centroid_L, k=1) 
    paircentroid_T = np.array(centroid_T)[pairindex_T] 
    blocker = []
    for Tcoor, Icoor, r in tqdm(zip(centroid_L, paircentroid_T, dis), total=len(centroid_L)):
        set1 = set(STree.query_ball_point(Tcoor, r))
        set2 = set(STree.query_ball_point(Icoor, r))
        blocker.append(len(set1 & set2))
    globalGraph.vs[lymphIDs]['stromaBlocker'] = blocker
