...
edgeDictReverse = {v:k for k, v in edgeDict.items()}
...
typeDictReverse = {v:k for k,v in typeDict.items()}

colormap = mpl.cm.get_cmap('tab20')
cellType_RGB = [
                (0  ,   0,   0), # nolabe no color 
                (0,    255,  0), # epithelial, green
                (233,  76,  36), # lymphocyte, yellow, 
                (23,  190, 207), # fibroblast, cyan
                (98,  72,  230), # macrophage, blue
                (231, 138, 195), # vascular, pink
                ] 
...
edge_RGB[0] = (255, 0, 0) # T-T
edge_RGB[1] = (98,  72,  230) # T-M
edge_RGB[2] = (255,  217,  47) # T-L
edge_RGB[3] = (255, 255, 255) # T-V
edge_RGB[4] = (23,  190, 207) # T-F
edge_RGB[6] = (98,  72,  230) # M-L
edge_RGB[9] = (255, 0, 0) # L-L
edge_RGB[10] = (255, 255, 255) # L-V
edge_RGB[11] = (23,  190, 207) # L-F
...
class make_graph_img():
    ...
    def read_csv(self, 
        ...
        self.specell_names = specell_names
        wsi_path = os.path.join(wsi_path, f'{sample_name}.svs')
        feature_path = os.path.join(feature_path, sample_name)
        ...
        if specell_names is None:
            ...
            for specell_name in specell_names:
                ...
                window_bbox.append([specell_centroid - bbox_size/2, specell_centroid + bbox_size/2])
        ...
        try:
            self.img = [wsi.read_region(location=i[0].astype('int'), level=0,size=(i[1]-i[0]).astype('int')) for i in window_bbox]
        ...
        omit_cell = []
        if len(select_cell) > 0:
            select_cell = [typeDictReverse[i] for i in select_cell]
            omit_cell = list(set(cellType) - set(select_cell))
        ...
        if len(select_edge) > 0:
            omit_edge = list(set(edgeDict.values()) - set(select_edge))
    ...
    def make_img_circle(self, nuclear_size = 5):
        def draw_circle(img, bbox, myclass, radius=nuclear_size):
    ...
    def make_img_line(self, thickness=5):
        def draw_line(img, bbox, myclass):
            ...
            if myclass not in edgeDictReverse.keys():
                c1 =  myclass.split('-')[0]
                c2 =  myclass.split('-')[1]
                myclass = f'{c2}-{c1}'
    ...
    def make_img(self, line_size=5, rect_size=5, nuclear_size = 5):
        self.img = self.rawimg.copy()
        self.make_img_line(line_size)
        self.make_img_circle(nuclear_size)
    ...
    def write_img(self):
        ...
        for i in range(len(self.window_bbox)):
            ...
            cell_name = self.specell_names[i]
            cv2.imwrite(f'{self.save_path}/{self.sample_name}_plot_{cell_name}.jpg', self.img[i][:, :, ::-1])
